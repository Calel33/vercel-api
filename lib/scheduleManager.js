...
// Use a consistent path that works for both web service and cron job
// Check if we're in the vercel-api directory or need to navigate to it
const isInVercelApi = __dirname.includes('vercel-api');
const SCHEDULES_FILE = isInVercelApi 
    ? path.join(__dirname, '..', 'schedules-data.json')
    : path.join(process.cwd(), 'vercel-api', 'schedules-data.json');
...
function loadSchedulesData() {
    try {
        console.log('🔍 Schedule Manager Debug:');
        console.log('📁 __dirname:', __dirname);
        console.log('📁 process.cwd():', process.cwd());
        console.log('📁 isInVercelApi:', isInVercelApi);
